...
class Instance:
    ...
    def memory_record(self, memory):
        self.memory_partitions.append(memory)
    ...
    if type == 'inference': # non-llm model execute best-fit allocation algorithm
            ...
            for node, gpus_on_node in node_memory_groups.items():
                ...
                if total_available_memory >= memory:
                    ...
                    for gpu in gpus_on_node:
                        ...
                        instance.memory_record(alloc_memory)
                    found = True
                    break
        ...
        if not allocated_gpus:
            for node_ip in nodes_info:  # Iterate over all possible node IPs directly from nodes_info
                new_gpus_on_node = [gpu for gpu in new_gpus if gpu.ip_address == node_ip['ip']]
                # new_gpus_on_node = new_gpus[:4]
    ...
    with lock:
        for gpu in active_gpus:
            if instance_id in gpu.instances: 
                ...
                if instance_data['type'] == 'llm-inference':
                    if len(instance.deployed_gpus)>1:
                        found_index = -1
                        for j, tmp_gpu in enumerate(instance.deployed_gpus):
                            if tmp_gpu==gpu:
                                found_index = j 
                                break
                        gpu.current_memory -= instance.memory_partitions[found_index]
                    else:
                        gpu.current_memory -= instance.memory[0]
                elif instance_data['type'] == 'inference':
                    gpu.current_memory -= instance.memory[0]
                elif instance_data['type'] == 'training':
                    gpu.current_memory -= instance.memory[0]
                found = True
...
if __name__ == '__main__':
    ...
    with open("instances-3200.txt", "r") as f:
        lines = f.readlines()
    ...
    for event in instances:
        ...
        if max_nums >=660 and  max_nums <=664 :
    ...
    plt.xticks()
    plt.tight_layout()
    plt.savefig('dilu-timeline.png', dpi=300)
